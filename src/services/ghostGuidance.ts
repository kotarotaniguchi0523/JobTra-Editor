...
export function analyzeGhostContext(text: string, cursorPosition: number): GhostGuidance {
  ...
  const hasResultKeywords = /(その結果|成果を|達成し|学びを得|評価をいた|改善され|向上しまし)/.test(activeContent);
  const hasContributionKeywords = /(貴社におい|入社後|貢献いた|活かして|努めてまい)/.test(activeContent);
  ...
  const hasActionKeywords = /(そこで私|取り組みとして|工夫として|施策として|提案し|働きかけ|構築し|徹底し)/.test(activeContent);
  ...
  const hasConclusionKeywords = /(私の強み|注力した|力を注ぎ|成し遂げ|最も打ち込)/.test(activeContent);
