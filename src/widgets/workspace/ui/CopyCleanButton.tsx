  ...
  return (
    <form action={submitCopy}>
      <button
        ...
        className={`flex shrink-0 cursor-pointer items-center gap-1 rounded-md px-2.5 py-1.5 text-xs font-medium whitespace-nowrap transition-all xl:gap-1.5 xl:px-3 ${
          isSuccess
      ...
      >
        {isPending ? (
          <>
            ...
            <span className="hidden xl:inline">コピー中...</span>
          </>
        ) : isSuccess ? (
          <>
            <Check className="h-3.5 w-3.5 shrink-0" />
            <span className="hidden xl:inline">コピー完了</span>
          </>
        ) : (
          <>
            <Copy className="h-3.5 w-3.5 shrink-0" />
            <span className="hidden xl:inline">{isError ? '再試行' : '提出用にコピー'}</span>
          </>
        )}
      </button>
    </form>
