...
interface WriteToolbarState {
  isFocusSentenceEnabled: boolean;
  isTypewriterScrollEnabled: boolean;
}

type WriteToolbarAction = { type: 'toggle-focus-sentence' } | { type: 'toggle-typewriter-scroll' };

const INITIAL_WRITE_TOOLBAR_STATE: WriteToolbarState = {
  isFocusSentenceEnabled: false,
  isTypewriterScrollEnabled: false,
};

function writeToolbarReducer(
  state: WriteToolbarState,
  action: WriteToolbarAction,
): WriteToolbarState {
  switch (action.type) {
    case 'toggle-focus-sentence':
      return { ...state, isFocusSentenceEnabled: !state.isFocusSentenceEnabled };
    case 'toggle-typewriter-scroll':
...
export const WriteToolbar: React.FC<WriteToolbarProps> = memo(({ onCleanFormatting, children }) => {
  const [state, dispatch] = useReducer(writeToolbarReducer, INITIAL_WRITE_TOOLBAR_STATE);
    ...
    <>
      <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-neutral-600">
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
          <button
            type="button"
            onClick={() => dispatch({ type: 'toggle-focus-sentence' })}
          ...
          <button
            type="button"
            onClick={() => dispatch({ type: 'toggle-typewriter-scroll' })}
            className={`flex cursor-pointer items-center gap-1 rounded border px-2 py-1 text-xs font-medium transition-colors sm:gap-1.5 sm:px-2.5 ${
              state.isTypewriterScrollEnabled
                ? 'border-neutral-900 bg-neutral-900 text-white'
                : 'border-neutral-200 bg-white text-neutral-700 hover:bg-neutral-100'
            }`}
...
WriteToolbar.displayName = 'WriteToolbar';
