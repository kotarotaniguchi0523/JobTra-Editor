...
export const Editor: React.FC<EditorProps> = memo(
  ({
      ...
      startTransition(() => {
        if (!draft.starBlocks) {
          ...
          const initialBlocks: StarBlocks = {
            conclusion:
              sentences.slice(0, Math.max(1, Math.floor(total * 0.2))).join('。') +
              (sentences.length > 0 ? '。' : ''),
            situation:
              sentences
                .slice(Math.max(1, Math.floor(total * 0.2)), Math.max(2, Math.floor(total * 0.4)))
                .join('。') + (sentences.length > 1 ? '。' : ''),
            action:
              sentences
                .slice(Math.max(2, Math.floor(total * 0.4)), Math.max(3, Math.floor(total * 0.7)))
                .join('。') + (sentences.length > 2 ? '。' : ''),
            result:
              sentences
                .slice(Math.max(3, Math.floor(total * 0.7)), Math.max(4, Math.floor(total * 0.85)))
                .join('。') + (sentences.length > 3 ? '。' : ''),
            contribution:
              sentences.slice(Math.max(4, Math.floor(total * 0.85))).join('。') +
              (sentences.length > 4 ? '。' : ''),
          };
    ...
    const handleInsertPhrase = useCallback(
      (phrase: string) => {
        const next =
          state.content.slice(0, state.cursorPos) + phrase + state.content.slice(state.cursorPos);
    ...
    const handleSaveSnapshot = useCallback(
      (label: string) => {
        const newSnapshot: DraftSnapshot = {
          ...
          label:
            label.trim() ||
            `${new Date().toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })} 時点の版`,
